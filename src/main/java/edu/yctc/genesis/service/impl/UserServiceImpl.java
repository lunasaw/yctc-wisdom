...
import edu.yctc.genesis.warpper.MailWarpper;
...
public class UserServiceImpl implements UserIService {
    ...
    public ResultDO<Void> updatePassword(UserDO userDO, String oldPassword, String newPassword) {
        try {
            UserDO userDOTemp = userDAO.getUserDOByNumberAndPassword(userDO.getNumber(),
                SHA256Util.SHA256(oldPassword));//通过账户  旧密码查询用户
            if (userDOTemp == null) {
                // 老密码核验错误
    ...
    //通过userDO返回学院名称
    @Override
    public ResultDO<String> getAcademyByUserDO(UserDO userDO) {
        if (userDO == null) {
            LOG.error("get academy by userDO fail, parameter invalid, userDO={}", userDO);
        ...
        try {
            ...
            if (academyId <= 0) {
                LOG.error("get academy by userDO fail, academyId <= 0, academyId={}, userDO={}", academyId, userDO);
                return new ResultDO<String>(false, ResultCode.PARAMETER_INVALID, ResultCode.MSG_PARAMETER_INVALID,
                    null);
            }
            String academy = academyDAO.getAcademyById(academyId);
            if (StringUtils.isBlank(academy)) {
                LOG.error("get academy by userDO fail, academyId={}, academy={}, userDO={}", academyId, academy,
                    userDO);
                return new ResultDO<String>(false, ResultCode.PARAMETER_INVALID, ResultCode.MSG_PARAMETER_INVALID,
                    null);
            }
            LOG.info("get academy by userDO success, academy={}, userDO={}", academy, userDO);
            return new ResultDO<String>(true, ResultCode.SUCCESS, ResultCode.MSG_SUCCESS, academy);
        } catch (Exception e) {
            LOG.error("get academy by userDO error, userDO={}", userDO, e);
    ...
    //通过学号/工号返回学生详细信息
    @Override
    public ResultDO<UserInformationVO> getInformationByUserId(long userId) {
        try {
            UserDO userDO = userDAO.getUserDOById(userId);
            if (userDO == null) {
                LOG.info("getUserDOById error, no such userDO,userId={}", userId);
                return new ResultDO<UserInformationVO>(false, ResultCode.NO_SUCH_USER, ResultCode.MSG_NO_SUCH_USER,
                    null);
            }
            LOG.info("getUserDOById success, userId={}", userId);

            long academyId = userDO.getAcademyId();
            String academyName = academyDAO.getAcademyById(academyId);
            if (StringUtils.isBlank(academyName)) {
                LOG.info("get academy by academyId error, no such academy, academyId={}", academyId);
                return new ResultDO<UserInformationVO>(false, ResultCode.DATABASE_CAN_NOT_FIND_DATA,
                    ResultCode.MSG_DATABASE_CAN_NOT_FIND_DATA, null);
            }
            LOG.info("getAcademyById success, academyName={}", academyName);

            long classId = userDO.getClassId();
            String className = classDAO.getNumberById(classId);
            if (StringUtils.isBlank(className)) {
                LOG.info("get classNumber by classId error, no such className, classId={}", classId);
                return new ResultDO<UserInformationVO>(false, ResultCode.DATABASE_CAN_NOT_FIND_DATA,
                    ResultCode.MSG_DATABASE_CAN_NOT_FIND_DATA, null);
            }
            LOG.info("getclassNameById success, className={}", className);

            UserInformationVO userInformationVO = new UserInformationVO();
            userInformationVO.setUserDO(userDO);
            userInformationVO.setAcademyName(academyName);
            userInformationVO.setClassName(className);

            LOG.info("get information by userId success, userId={}, userInformationVO={}", userId, userInformationVO);
            return new ResultDO<UserInformationVO>(true, ResultCode.SUCCESS, ResultCode.MSG_SUCCESS, userInformationVO);
        } catch (Exception e) {
            LOG.info("get information by userId error, userId={}", userId, e);
            return new ResultDO<UserInformationVO>(false, ResultCode.ERROR_SYSTEM_EXCEPTION,
