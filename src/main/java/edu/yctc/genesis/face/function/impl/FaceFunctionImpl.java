package edu.yctc.genesis.face.function.impl;
...
import edu.yctc.genesis.face.util.BodyRectangle;
import edu.yctc.genesis.face.util.CameraControl;
...
import edu.yctc.genesis.face.util.Model;
import edu.yctc.genesis.face.util.MyFaceApi;
import edu.yctc.genesis.face.util.OcrContorl;
import edu.yctc.genesis.face.util.PicCut;
import edu.yctc.genesis.face.util.SpringContextUtils;
import edu.yctc.genesis.face.util.WaitForDate;
...
import edu.yctc.genesis.service.impl.AttendanceServiceImpl;
import edu.yctc.genesis.service.impl.ClassQualityServiceImpl;
import edu.yctc.genesis.service.impl.DetailsServiceImpl;
import edu.yctc.genesis.service.impl.KnowledgeServiceImpl;
import edu.yctc.genesis.service.impl.LessonServiceImpl;
...
public class FaceFunctionImpl implements FaceFunction {
    ...
    @Override
    public void alterFaceToken(UserDO userDO) {
        /** 存放人脸 */
        List<Face> faces = new ArrayList<>();

        /** 识别 */
        MyFaceApi.myDetectFromUrl(userDO.getPicture(), null, faces);

        /** 等待人脸检测完成 */
        WaitForDate.waitForDate(faces);

        /** 保存到face++平台的faceset */
        ResultDO<String> academyResult = userService.getAcademyByUserDO(userDO);
        if (academyResult.getModule() != null) {
            MyFaceApi.myFaceSetAdd(academyResult.getModule(), faces.get(0).getFaceToken());
        } else {
            LOG.error("Colleges don't exist!, userDO={}", userDO);
            return;
        }

        /** 更新数据库的face_token */
        userDO.setFaceToken(faces.get(0).getFaceToken());
        userService.updateUserDO(userDO);
    }
